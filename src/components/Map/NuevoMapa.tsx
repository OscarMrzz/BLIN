...
*/
// const start = { name: "Mezapa", lng: -87.66929792025022, lat: 15.603563320676374 };
// const end = { name: "Progreso", lng: -87.8065947301114, lat: 15.402027212485665 };
...
interface MapProps {
  ...
  miPisicion?: { name: string; lng: number; lat: number };
}

export function NuevoMapa({ start, end, miPisicion }: MapProps) {
  ...
  return (
    <div className="h-[500px] w-full relative">
      <Map
        center={[
          miPisicion?.lng || start?.lng || 0,
          miPisicion?.lat || start?.lat || 0,
        ]}
        zoom={16}
        theme="light"
      >
        ...
        {miPisicion && (
          <MapMarker longitude={miPisicion.lng} latitude={miPisicion.lat}>
            <MarkerContent>
              <div className="flex flex-col items-center">
                <MarcadorMapa size={32} Styles="text-red-500" />
                <MarkerLabel position="top">{miPisicion.name}</MarkerLabel>
              </div>
            </MarkerContent>
          </MapMarker>
        )}
