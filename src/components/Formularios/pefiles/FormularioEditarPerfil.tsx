...
import { updatePerfil } from "@/lib/services/perfilesServices";
...
type Props = {
  ...
  refrescarTabla: () => void;
  perfilAEditar: PerfilesInterface;
};
...
export function FormularioEditarPerfil({
  ...
  const [formData, setFormData] = React.useState({
    ...
    apellido: perfilAEditar.apellido || "",
    dni: perfilAEditar.dni?.toString() || "",
    id_roles: perfilAEditar.id_roles || "",
    id_user: perfilAEditar.id_user || "",
    id_rutas: perfilAEditar.id_rutas || "",
  });
  ...
  const handleSubmit = async (evento: React.FormEvent) => {
    ...
    try {
      await updatePerfil(perfilAEditar.id_perfiles, {
        ...
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
          <DialogHeader className="mb-4 flex-shrink-0">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Modifique la información del perfil seleccionado.
            </p>
          </DialogHeader>
          ...
          <DialogFooter className="pt-4 border-t flex-shrink-0">
            ...
            <Button
              type="submit"
              className="h-10 px-6 bg-amber-600 hover:bg-amber-700 text-white font-medium"
            >
              Actualizar Perfil
            </Button>
