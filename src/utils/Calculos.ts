...
export const obtenerMinutosParaLlegada = (
  ...
  const tiempoTransito = (miParada.distanciaDesdeOrigen / ruta.velocidadPromedioKmh) * 60;

  // 2. Buscar la salida cuyo (despacho + viaje) sea mayor a la hora actual
  const proximaSalida = ruta.horariosRuta.find(salida => (salida + tiempoTransito) > minutosActuales);

  if (!proximaSalida) {
    console.log("❌Ya no hay mas buses en tu posicion")
    return null}; // Ya no pasan buses hoy
